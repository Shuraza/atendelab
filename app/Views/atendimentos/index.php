            ...
            <div class="row g-3">
                ...
                <div class="col-12">
                    ...
                    <textarea class="form-control" name="descricao" rows="3" required
                        ...
                        title="Descreva o atendimento (mínimo 5 caracteres)."></textarea>
                </div>
            ...
            <form id="formStatus">
                <div class="modal-body">
                    ...
                    <div>
                        ...
                        <textarea class="form-control" name="observacao_final" rows="3" maxlength="2000"
...
async function carregarAtendimentos() {
    try {
        ...
        tbody.innerHTML = atendimentos.map(atendimento => {
            // Os nomes reais retornados por AtendimentosController::listar() são
            // pessoa_nome, tipo_nome e responsavel_nome (JOIN com pessoas, tipos_atendimentos e usuarios).
...
// A observação final só é obrigatória quando o status escolhido é
// "concluido" — mesma regra aplicada em AtendimentosController::alterarStatus().
function atualizarObrigatoriedadeObservacao() {
    const statusSelecionado = document.querySelector('#formStatus [name="status"]').value;
    const campoObservacao = document.querySelector('#formStatus [name="observacao_final"]');
    campoObservacao.required = statusSelecionado === 'concluido';
    campoObservacao.placeholder = statusSelecionado === 'concluido'
        ? 'Obrigatória para concluir o atendimento'
        : 'Opcional';
}

document.querySelector('#formStatus [name="status"]').addEventListener('change', atualizarObrigatoriedadeObservacao);
...
function abrirStatus(id, status) {
    ...
    atualizarObrigatoriedadeObservacao();
    statusModal().show();
}
